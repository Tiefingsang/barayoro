...
<div :class="[$store.app.menu=='horizontal' ? 'max-w-[1704px] mx-auto xxl:px-0 xxl:pt-8':'',$store.app.stretch?'xxxl:max-w-[92%] mx-auto':'']" class="p-3 md:p-4 xxl:p-6 space-y-4 xxl:space-y-6">
        ...
        <div class="white-box xxxl:p-6">
          <div class="n20-box xxxl:p-6 relative ltr:bg-right rtl:bg-left bg-no-repeat max-[650px]:!bg-none bg-contain" style="background-image: url({{ asset('assets/images/breadcrumb-el-1.png') }})">
            <h2 class="mb-3 xxxl:mb-5">Créer une facture</h2>
            <ul class="flex flex-wrap gap-2 items-center">
              ...
              <li class="text-sm text-neutral-100">•</li>
              <li>
                <a class="flex items-center gap-2 text-primary-300" href="#"> <i class="las la-plus-circle shrink-0"></i> <span>Créer</span></a>
              </li>
            </ul>
          </div>
        </div>
        ...
        <form method="POST" action="{{ route('invoices.store') }}">
            @csrf
        <div class="white-box">
          <h4 class="bb-dashed-n30">Créer une facture</h4>
            <div
                x-data="{
                invoiceProducts: [],
                nextId: 1,
...
<div class="ltr:sm:pl-5 rtl:sm:pr-5 xxl:ltr:pl-10 xxl:rtl:pr-10">
  <div x-data="{ open: false, selectedClient: null, clientSearch: '' }">
  ...
  @error('client_id') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            ...
            <div class="n20-box xl:p-6 grid grid-cols-12 gap-4 xxl:gap-6 mb-7 xxl:mb-10">
              <div class="col-span-12 md:col-span-6 xl:col-span-3">
                <div class="relative flex items-center gap-4 rounded-full border border-neutral-40 px-4 dark:border-neutral-500 lg:px-6">
                  ...
                  <input type="text" placeholder="Auto-généré" class="w-full bg-transparent py-2.5 lg:py-3.5" disabled />
                </div>
              </div>
              <div class="col-span-12 md:col-span-6 xl:col-span-3">
                <div x-data="{isOpen:false,selected:'pending', items:['draft','sent','pending','paid']}" class="relative">
              ...
              <div class="col-span-12 md:col-span-6 xl:col-span-3">
                <div class="relative flex items-center gap-4 rounded-full border border-neutral-40 px-4 dark:border-neutral-500 lg:px-6">
                  ...
                  <input type="date" name="issue_date" value="{{ date('Y-m-d') }}" class="w-full bg-transparent py-2.5 lg:py-3.5" required />
                </div>
              </div>
              <div class="col-span-12 md:col-span-6 xl:col-span-3">
                <div class="relative flex items-center gap-4 rounded-full border border-neutral-40 px-4 dark:border-neutral-500 lg:px-6">
                  <label class="absolute -top-2 bg-neutral-20 px-2 text-xs dark:bg-neutral-903">Date d'échéance</label>
                  <input type="date" name="due_date" value="{{ date('Y-m-d', strtotime('+30 days')) }}" class="w-full bg-transparent py-2.5 lg:py-3.5" required />
                </div>
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
              <div class="relative rounded-xl border border-neutral-30 px-4 py-3 dark:border-neutral-500">
                ...
                <textarea name="notes" rows="3" class="w-full bg-transparent py-2 resize-none" placeholder="Notes supplémentaires..."></textarea>
              </div>
              <div class="relative rounded-xl border border-neutral-30 px-4 py-3 dark:border-neutral-500">
                <label class="absolute -top-2 left-5 bg-neutral-0 px-2 text-xs dark:bg-neutral-904">Conditions</label>
                <textarea name="terms" rows="3" class="w-full bg-transparent py-2 resize-none" placeholder="Conditions de paiement..."></textarea>
              </div>
            </div>

            <div class="flex gap-4 flex-wrap xxl:gap-6">
              <button type="submit" class="btn-primary">Créer la facture</button>
              <button type="button" class="btn-primary-outlined" onclick="window.location.href='{{ route('invoices.index') }}'">Annuler</button>
            </div>
          </div>
        </div>
        </form>
